...
from model import Credentials, OCIOGlassUser
...
def get_ocio_user(userid):
    ocio_user = OCIOGlassUser.get_by_key_name(userid)

    if hasattr(ocio_user, "user_id") and ocio_user.user_id == userid: # existing user
      logging.info("Got OCIOGlassUser ID: %s " % str(ocio_user.user_id))

    else: # new user
      logging.info("Creating new UserID %s" % userid)
      ocio_user = OCIOGlassUser(key_name=userid, user_id=userid)
      ocio_user.put()

    ocio_user.last_accessed = datetime.now()
